	...
	fetch("../translation.json")
		...
		.then((data) => {
			...
			footer.children[0].innerHTML =
				data.footer[`${isPortuguese ? "br" : "us"}`] // FOOTER

			for (const topic of topics) {
				// EACH TOPIC
				if (data.topics[i].title != undefined) {
					topic.children[0].children[1].innerHTML =
						data.topics[i].title[`${isPortuguese ? "br" : "us"}`]
					if (topic.children[1].children[0].localName == "ul") {
						j = 0
						for (const list of topic.children[1].children[0]
							.children) {
								...
								if (list.children[0].children.length > 0) {
									if (list.children[0].children[1]) {
										list.children[0].children[1].innerHTML =
											data.topics[i].itens[j].key[
												`${isPortuguese ? "br" : "us"}`
											]
									} else {
										list.children[0].children[0].innerHTML =
											data.topics[i].itens[j].key[
												`${isPortuguese ? "br" : "us"}`
											]
									}
						...
						topic.children[1].children[0].localName == "destaque"
					) {
						topic.children[1].innerHTML =
							data.topics[i].itens[0].key[
								`${isPortuguese ? "br" : "us"}`
							]
					}
				}
